...
```{r}
# Filtrage des N/A
landcover_groups_clean <- landcover_groups_comp[!is.na(N_obs) & !is.na(N_exp)]
# Test du Khi²
khi2_groups <- chisq.test(x = landcover_groups_clean$N_obs, 
                        p = landcover_groups_clean$N_exp / sum(landcover_groups_clean$N_exp))
khi2_groups
```
...
```{r}
# Calcul de la contribution des classes au khi²
landcover_groups_clean[, contrib_khi2_groups := (khi2_groups$stdres)^2]
landcover_groups_clean |> data.table()
```


```{r}
# Visualiser les contributions en %
landcover_groups_clean[, contrib_khi2_groups_perc := 100 * contrib_khi2_groups / sum(contrib_khi2_groups)]
ggplot(landcover_groups_clean, aes(x = contrib_khi2_groups_perc, 
                      y = reorder(landcover_groups, contrib_khi2_groups_perc))) +
  geom_col(fill = "tomato") +
  labs(x = "Contribution au khi² (%)",
       title = "Contribution de chaque classe landcover au test du khi²") +
  theme_minimal(base_size = 14)
```
...
```{r}
# Pour chaque classe, est-ce qu'elle est sur- ou sous-représentée dans les données ?
landcover_groups_clean[, deviation := khi2_groups$stdres]
ggplot(landcover_groups_clean, aes(x = deviation, 
                      y = reorder(landcover_groups, deviation))) +
  geom_col(aes(fill = deviation > 0)) +
  scale_fill_manual(values = c("steelblue", "tomato"), 
                    labels = c("Sous-représentée", "Sur-représentée")) +
  labs(x = "Résidu standardisé",
       title = "Sur- ou sous-représentation des classes landcover") +
  theme_minimal(base_size = 14)
```

On voit donc qu'il y a de forts biais spatiaux dans l'échantillonnage, certains types de milieu sont préférentiellement visités, la distribution des observations n'est pas conforme à la proportion de chaque type de milieu sur le territoire français.
Rq : on voit que la catégorie "Autres" a un poids important dans le khi², faut-il la retirer ?
...
gens_exp |> datatable()
```
Rq : Agréger les GEnS en GEnZname ?

```{r}
# Barplot
ggplot(gens_exp) +
  geom_col(aes(x = perc_exp, 
               y = reorder(GEnZname, perc_exp)), fill = "darkorange") +
...
gens_obs <- dat[, .(N_obs = .N), by = GEnS_v3_bioclim]
...
```{r}
# Avoir le même nom de variable pour les labels des zones bioclimatiques
setnames(gens_exp, "GEnZname", "GEnS_v3_bioclim")
names(gens_exp)
```

```{r}
# Joindre les 2 distributions par les labels des zones bioclimatiques
gens_comp <- gens_obs[gens_exp, on = .(GEnS_v3_bioclim)]
gens_clean <- gens_comp[!is.na(perc_obs)]
gens_clean |> datatable()
```

```{r}
# Transformer la table pour pouvoir la représenter graphiquement : format large --> format long
gens_long <- melt(gens_clean,
...
```{r}
# Filtrage des N/A
gens_clean <- gens_clean[!is.na(N_obs) & !is.na(N_exp)]
# Test du Khi²
khi2_gens <- chisq.test(x = gens_clean$N_obs, 
                        p = gens_clean$N_exp / sum(gens_clean$N_exp))
khi2_gens
```
...
```{r}
# Calcul de la contribution des zones bioclimatiques au khi²
gens_clean[, contrib_khi2 := (khi2_gens$stdres)^2]
gens_clean |> data.table()
```

```{r}
# Contribution au total du khi² (%)
gens_clean[, contrib_khi2_perc := 100 * contrib_khi2 / sum(contrib_khi2)]

# Graphique
ggplot(gens_clean, aes(x = contrib_khi2_perc, 
                       y = reorder(GEnS_v3_bioclim, contrib_khi2_perc))) +
  geom_col(fill = "tomato") +
  labs(x = "Contribution au khi² (%)",
       title = "Contribution de chaque zone bioclimatique au test du khi²") +
  theme_minimal(base_size = 14)
```


```{r}
# Pour chaque classe, est-ce qu'elle est sur- ou sous-représentée dans les données ? (calcul de la déviation)
gens_clean[, deviation := khi2_gens$stdres]
ggplot(gens_clean, aes(x = deviation, 
