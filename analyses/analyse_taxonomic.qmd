...
# Nettoyage : supprimer la colonne temporaire correctedName + supprimer les identifications autres qu'au niveau espèce
dat[, correctedName := NULL]
dat <- dat[grepl(" ", scientificName)]
...
### Tableau de décision basé sur critère de >10 sites

```{r}
# Comptage sites par région et année
...
# tableau gt avec surbrillance cellules >= 10
gt_table <- gt(tab_sites_wide) %>%
  tab_header(title = "Nombre de sites STELI par région et par an")
for (col in as.character(2011:2024)) {
  gt_table <- gt_table %>%
    tab_style(
      style = cell_fill(color = "lightgreen"),
...
### Tableau de décision basé sur critère de >50 sessions

```{r}
# Comptage sessions par région et année
tab_sites <- dat_steli[, .(n_sites = uniqueN(id_ses)), by = .(region, year = year(eventDate))]
...
# tableau gt avec surbrillance cellules >= 200
gt_table <- gt(tab_sites_wide) %>%
  tab_header(title = "Nombre de sessions par région et par an")
for (col in as.character(2011:2024)) {
  gt_table <- gt_table %>%
    tab_style(
      style = cell_fill(color = "lightgreen"),
...
### Hauts-de-France (2011-2017)

```{r}
# Filtrage
dat_hdf <- dat[region %in% c("Hauts-de-France") & eventDate >= as.IDate("2011-01-01") & eventDate <= as.IDate("2017-12-31")]
...
### Auvergne-Rhône-Alpes (2013–2019) --> modifier les dates en fonction du critère de sélection...
...
### Région : Nouvelle-Aquitaine (2012–2019) --> modifier les dates en fonction du critère de sélection...
...
```{r}
# Filtrage
dat_steli_hdf <- dat[source == "STELI" & region == "Hauts-de-France" & eventDate >= as.IDate("2011-01-01") & eventDate <= as.IDate("2017-12-31")]
...
ggplot(sessions_per_month, aes(x = month, y = factor(year), fill = n_sessions)) +
  ...
  labs(title = "Nombre de sessions STELI par mois et par année",
...
ggplot(sessions_monthly, aes(x = month, y = n_sessions)) +
  ...
  labs(title = "Sessions STELI par mois (total sur 2011–2019)",
...
```{r}
# Filtrage
dat_steli_norm <- dat[source == "STELI" & region == "Normandie" & eventDate >= as.IDate("2012-01-01") & eventDate <= as.IDate("2013-12-31")]

# Nombre de sessions par mois et par année
dat_steli_norm[, `:=`(year = year(eventDate), month = month(eventDate, label = TRUE, abbr = TRUE))]
sessions_per_month <- dat_steli_norm[, .(n_sessions = uniqueN(id_ses)), by = .(year, month)]
...
# Séries mensuelles
sessions_monthly <- dat_steli_norm[, .(n_sessions = uniqueN(id_ses)), by = month]
ggplot(sessions_monthly, aes(x = month, y = n_sessions)) +
  geom_col(fill = "steelblue") +
  labs(title = "Sessions STELI par mois (total sur 2011–2019)",
       x = "Mois", y = "Nombre total de sessions") +
...
```{r}
# Filtrage
dat_steli_ge <- dat[source == "STELI" & region == "Grand Est" & eventDate >= as.IDate("2015-01-01") & eventDate <= as.IDate("2017-12-31")]
...
```{r}
# Filtrage
dat_steli_ara <- dat[source == "STELI" & region == "Auvergne-Rhône-Alpes" & eventDate >= as.IDate("2013-01-01") & eventDate <= as.IDate("2019-12-31")]

# Nombre de sessions par mois et par année
dat_steli_ara[, `:=`(year = year(eventDate), month = month(eventDate, label = TRUE, abbr = TRUE))]
sessions_per_month <- dat_steli_ara[, .(n_sessions = uniqueN(id_ses)), by = .(year, month)]
...
# Séries mensuelles
sessions_monthly <- dat_steli_ara[, .(n_sessions = uniqueN(id_ses)), by = month]
ggplot(sessions_monthly, aes(x = month, y = n_sessions)) +
  geom_col(fill = "steelblue") +
  labs(title = "Sessions STELI par mois (total sur 2011–2019)",
       x = "Mois", y = "Nombre total de sessions") +
...
### Nouvelle-Aquitaine (2012–2019)

```{r}
# Filtrage
dat_steli_na <- dat[source == "STELI" & region == "Nouvelle-Aquitaine" & eventDate >= as.IDate("2012-01-01") & eventDate <= as.IDate("2019-12-31")]

# Nombre de sessions par mois et par année
dat_steli_na[, `:=`(year = year(eventDate), month = month(eventDate, label = TRUE, abbr = TRUE))]
sessions_per_month <- dat_steli_na[, .(n_sessions = uniqueN(id_ses)), by = .(year, month)]
...
# Séries mensuelles
sessions_monthly <- dat_steli_na[, .(n_sessions = uniqueN(id_ses)), by = month]
ggplot(sessions_monthly, aes(x = month, y = n_sessions)) +
  geom_col(fill = "steelblue") +
  labs(title = "Sessions STELI par mois (total sur 2011–2019)",
       x = "Mois", y = "Nombre total de sessions") +
...
```{r}
# Filtrage
dat_steli_occ <- dat[source == "STELI" & region == "Occitanie" & eventDate >= as.IDate("2016-01-01") & eventDate <= as.IDate("2018-12-31")]
