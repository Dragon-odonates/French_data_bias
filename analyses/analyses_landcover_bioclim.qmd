...
1.1 + 1.2 + 1.3 + 1.4 Zones urbanisées et artificielles
...
clc_expected[, landcover_groups := fcase(
  ...
  CLC2018_landcover %in% c("Inland marshes", "Peat bogs",  "Water courses", "Water bodies"),
...
Rq : Il serait pertinent pour des analyses ultérieures d'analyser la distribution du biais au sein des classes avec un fort biais observé.
...
Le test du khi² de conformité indique une différence hautement significative entre la distribution observée des classes d’occupation du sol et leur distribution attendue (X² = 982169, df = 8, p \< 2.2e-16). L'indice FEI (Frequency Effect Index) associé suggère une déviation globale très faible voir nulle par rapport à la distribution théorique (FEI = 0.03).
...
landcover_groups_comp |> data.table()
# Visualiser les contributions
...
gens_comp <- gens_obs[gens_exp, on = .(GEnS_v3_bioclim)][!is.na(N_obs) & !is.na(N_exp) & N_obs != 0 & N_exp != 0]
gens_comp |> datatable()
# Transformer la table pour pouvoir la représenter graphiquement : format large --> format long
...
Le test du khi² de conformité indique une différence significative entre la distribution observée des classes d’occupation du sol et leur distribution attendue (X² = 85207, df = 7, p-value < 2.2e-16). L'indice FEI (Frequency Effect Index) associé suggère une déviation globale nulle par rapport à la distribution théorique (FEI = 0.00).
...
```{r}
# Quelles zones sont sur- ou sous-représentées dans les données ?
gens_comp[, deviation2 := khi2_gens$stdres]
