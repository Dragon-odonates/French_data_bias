...
Le test du khi² de conformité indique une différence hautement significative entre la distribution observée des classes d’occupation du sol et leur distribution attendue (X² = 1377536, df = 40, p \< 2.2e-16). L'indice FEI (Frequency Effect Index) associé suggère une déviation globale très faible voir nulle par rapport à la distribution théorique (FEI = 0.0062).
...
On voit donc qu'il y a de forts biais spatiaux dans l'échantillonnage, certains types de milieu sont préférentiellement visités, la distribution des observations n'est pas conforme à la proportion de chaque type de milieu sur le territoire français. On va répéter cette analyse en agrégant les classes CLC en groupes plus lisibles pour présenter ces résultats.

## Etude du biais spatial d'occupation des sols (classes agrégées)

### Agrégation des classes CLC (méthode 1 : sur le data.table)

Pour le regroupement des classes clc, se référer à la page : 
https://land.copernicus.eu/content/corine-land-cover-nomenclature-guidelines/html/

Créer un tableau résumé de l'aggrégation à mettre en annexe du rapport.
...
khi2_groups
# Mesure de la taille de l'effet
fei(x = landcover_groups_comp$N_obs, 
                        p = landcover_groups_comp$N_exp / sum(landcover_groups_comp$N_exp))
```

-   Adjusted for uniform expected probabilities.
-   One-sided CIs: upper bound fixed at \[1.00\].!! Chercher pourquoi on a ce résultat

Le test du khi² de conformité indique une différence hautement significative entre la distribution observée des classes d’occupation du sol et leur distribution attendue (X² = 938190, df = 6, p \< 2.2e-16). L'indice FEI (Frequency Effect Index) associé suggère une déviation globale très faible voir nulle par rapport à la distribution théorique (FEI = 0.07).
...
ggplot(landcover_groups_comp, aes(x = deviation, 
  ...
  labs(x = "Résidu standardisé",
       title = "Sur- ou sous-représentation des groupes landcover") +
  theme_minimal(base_size = 14)
```

## Etude du biais spatial des zones bioclimatiques

### Distribution attendue

Quelle proportion du territoire métropolitain est est couverte par chaque zone bioclimatique ?

Données issues de Metzger et al. 2013 https://doi.org/10.1111/geb.12022

-   raster zones bioclimatiques mondiales (à cropper sur la France métropole) : https://datashare.ed.ac.uk/handle/10283/3089
...
meta_gens <- read.csv(here(read_folder, "GEnS_v3_classification.csv"))
# Reprojetter le vecteur pour le superposer au raster
fr_4326 <- project(fr, crs(gens))
# Couper le raster aux frontières de la métropole
gens_fr <- crop(gens, fr_4326, mask=TRUE)
# Comme on a un raster en lat/long, on calcule les statistiques avec expanse()
...
```{r}
# Afficher le résultat
bioclim_exp <- read.csv(file.path(read_folder, "france_GEnS_v3.csv"))
...
On va maintenant produire des graphiques et tableaux comparatifs des 2 distributions pour visualiser les biais, avant de tester statistiquement les différences de distribution des zones bioclimatiques.

```{r}
# Joindre les 2 distributions par les labels des zones bioclimatiques
gens_comp <- gens_obs[gens_exp, on = .(GEnS_v3_bioclim)][!is.na(N_obs) & !is.na(N_exp) & perc_obs >= 1 & perc_exp >= 1]
gens_comp |> datatable()
# Transformer la table pour pouvoir la représenter graphiquement : format large --> format long
...
Rq : probablement que la zone méditerranée est importante dans les données opportunistes (plus d'sp sous contraintes, peut-être plus de suivis dans ces zones là) = effet vacances ie odonatologues opportunistes font des suivis hors de leur zone d'origine.
...
Le test du khi² de conformité indique une différence significative entre la distribution observée des zones bioclimatiques et leur distribution attendue (X² = 30652, df = 2, p < 2.2e-16). Attention à l'interprétation de ce résultat, la grande taille d'échantillons rend le test très sensible, une p-value significative ne signifie pas forcément la présence une différence pertinente d'un point de vue écologique. L'indice FEI (Frequency Effect Index) associé suggère une déviation globale très faible par rapport à la distribution théorique (FEI = 0.04), elle ne reflète pas une différence écologique pertinente.
...
ggplot(gens_comp, aes(x = deviation, 
  ...
  scale_fill_manual(values = c("steelblue", "tomato"), 
                    labels = c("Sous-représentée", "Sur-représentée")) +
  labs(x = "Résidu standardisé",
       title = "Sur- ou sous-représentation des zones bioclimatiques") +
  theme_minimal(base_size = 14)
```

Ce graphique permet de savoir quelles zones bioclimatiques sont sur ou sous-représentées dans les données, mais c'est à remettre en contexte, le FEI montre que la différence entre les distributions observées et attendues n'est pas significative écologiquement parlant. C'est-à-dire qu'il y a un biais oui, mais il n'est pas assez important pour qu'on s'en inquiète réellement. A vérifier.  
